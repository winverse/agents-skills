...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-05-11 | `web-research` | Web research method hardened | `active -> critical` | query fan-out, source ledger, evidence scoring, stop rules, eval prompts, static validator 기준을 추가했다. | system validator, skill-specific validator | broad topic eval runner는 필요해지면 추가한다. |
| 2026-05-11 | `skill-to-html` | HTML guide standard hardened | `active -> active` | 단순 문서 분할이 아니라 decision matrix, flowchart, chart, resource map 중심으로 HTML을 만들도록 기준을 정리했다. | system validator, skill-specific validator | 새 skill이 늘면 visual pattern 예시를 보강한다. |
