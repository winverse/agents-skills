...
- Keep the `skill.html` all-skills catalog generated by `scripts/update-html-catalog.ts`; do not hand-edit the content between `skill-catalog:start` and `skill-catalog:end`.
...
5. If skill folders are added, removed, renamed, or their adjacent `skill.html` files change inventory state, refresh the generated HTML catalog:
...
6. If the user asks what to use for a task, inspect the output and answer with:
