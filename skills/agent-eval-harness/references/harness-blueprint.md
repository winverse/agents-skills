...
`workflow`는 raw idea, existing API cleanup, substantial UI, CLI/no-browser, MCP/API automation, project setup verification, completion/ship, goal condition quality 같은 lane을 가진다. 각 lane은 최소 하나의 routing case와 하나의 artifact hygiene case를 둔다.
