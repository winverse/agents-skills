...
.codex/hooks/cmux_pin_prompt.mjs
```
...
두 hook script는 실행 시점의 git root가 script가 들어 있는 repo root와 같은지 먼저 확인한다. root가 다르면 아무 작업도 하지 않고 조용히 종료한다. 따라서 승인 state가 전역 파일에 저장되어 있더라도, 이 repo 밖에서 잘못 실행된 hook이 `skill-to-html`, validator, cmux tab 변경을 수행하지 않는다.
...
type = "command"
command = 'node "$(git rev-parse --show-toplevel)/.codex/hooks/cmux_pin_prompt.mjs"'
timeout = 5
statusMessage = "Pinning prompt to cmux tab"
```
...
이 메시지는 정상이다. 현재 이 repo는 skill HTML/validation script를 `PostToolUse`에 연결하고, cmux session에서는 제출한 prompt 원문을 현재 tab과 `current-question` status에 고정하는 `UserPromptSubmit` helper도 연결한다.

- `PostToolUse`: 파일 편집 직후 `SKILL.md` 변경을 확인하고 stale guide를 자동 갱신한 뒤 검증한다.
- `UserPromptSubmit`: `.codex/hooks/cmux_pin_prompt.mjs`가 cmux 안에서 현재 prompt 원문을 tab title과 `current-question` status로 고정한다. 이 hook은 agent의 의미 요약을 만들지 않는다. 의미 기반 tab title이 필요하면 agent가 turn 시작 후 `cmux rename-tab`으로 덮어쓴다. 이 hook에서는 skill HTML 자동 갱신이나 repo validation을 실행하지 않는다.
...
- cmux prompt pinning hook은 `CMUX_SURFACE_ID`가 있으면 `cmux rename-tab --surface <surface-id> ...`를 먼저 사용하고, 필요할 때만 `cmux identify`의 `caller.tab_ref`를 fallback으로 사용한다. tab과 `current-question` status에는 제출된 prompt 원문을 같은 값으로 넣는다. cmux 밖이거나 prompt payload를 찾지 못하면 조용히 종료한다.
