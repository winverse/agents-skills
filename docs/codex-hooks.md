...
`.codex/config.toml`은 Codex hooks를 켠다. 세션 시작이나 resume 때는 `SessionStart` hook이 Caveman mode 활성 메시지를 출력한다. 파일 편집 계열 tool 사용 후에는 `PostToolUse` hook script가 실행된다. 현재 `PostToolUse` 설정은 `--auto` 모드로 동작해서 stale `skill.html`이 있으면 별도 `codex exec` 프로세스로 `skill-to-html`을 수행하고, 스킬 관련 변경 뒤 검증 명령을 실행한다. 읽기 전용 shell 명령이 파일을 자동 수정하지 않도록 `PostToolUse` matcher는 편집 계열 tool로 제한한다.
...
hook script는 실행 시점의 git root가 script가 들어 있는 repo root와 같은지 먼저 확인한다. root가 다르면 아무 작업도 하지 않고 조용히 종료한다. 따라서 승인 state가 전역 파일에 저장되어 있더라도, 이 repo 밖에서 잘못 실행된 hook이 `skill-to-html`이나 validator를 수행하지 않는다.
...
이 메시지는 정상이다. 현재 이 repo는 Caveman mode startup reminder를 `SessionStart`에 연결하고, skill HTML/validation script를 `PostToolUse`에 연결한다. Warp title fallback은 hook이 아니라 assistant 응답 첫 줄 formatting으로 처리한다.

- `SessionStart`: Codex startup/resume 때 Caveman mode 활성 메시지를 출력한다. 모드 적용 여부를 눈으로 확인하기 위한 reminder이며, 파일을 수정하지 않는다.
...
`SessionStart` hook은 startup/resume 이벤트에서 `CAVEMAN MODE ACTIVE...` 메시지만 출력한다.
