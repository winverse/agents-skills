...
`.codex/config.toml`은 Codex hooks를 켠다. 세션 시작이나 resume 때는 `SessionStart` hook이 `plugins/caveman/plugins/caveman/hooks/codex/sessionstart.mjs`를 실행해 Caveman skill 본문을 세션 컨텍스트로 출력한다. 파일 편집 계열 tool 사용 후에는 `PostToolUse` hook script가 실행된다. 현재 `PostToolUse` 설정은 `--auto` 모드로 동작해서 stale `skill.html`이 있으면 별도 `codex exec` 프로세스로 `skill-to-html`을 수행하고, 스킬 관련 변경 뒤 검증 명령을 실행한다. 읽기 전용 shell 명령이 파일을 자동 수정하지 않도록 `PostToolUse` matcher는 편집 계열 tool로 제한한다.
