...
이 메시지는 정상이다. 현재 이 repo는 skill HTML/validation script를 `PostToolUse`에 연결하고, cmux session에서는 제출한 prompt의 짧은 작업 라벨을 현재 tab에, 원문을 status에 고정하는 `UserPromptSubmit` helper도 연결한다.

- `PostToolUse`: 파일 편집 직후 `SKILL.md` 변경을 확인하고 stale guide를 자동 갱신한 뒤 검증한다.
- `UserPromptSubmit`: `.codex/hooks/cmux_pin_prompt.mjs`가 cmux 안에서 현재 prompt를 짧은 작업 라벨과 전체 `current-question` status로 고정한다. 이 hook에서는 skill HTML 자동 갱신이나 repo validation을 실행하지 않는다.
...
- cmux prompt pinning hook은 `CMUX_SURFACE_ID`가 있으면 `cmux rename-tab --surface <surface-id> ...`를 먼저 사용하고, 필요할 때만 `cmux identify`의 `caller.tab_ref`를 fallback으로 사용한다. tab에는 규칙 기반 작업 라벨만 넣고, 원문 전체는 `current-question` status에 보존한다. cmux 밖이거나 prompt payload를 찾지 못하면 조용히 종료한다.
