...
- For existing backend/API structure cleanup, read the current docs/tree/scripts/tests, write a boundary map, and use `project-structure` as an audit handoff before staged refactor issues. Do not force a full PRD when behavior is unchanged.
- For new service plus Docker/AWS/Pulumi requests, run Discovery and ADR before infra-aware structure generation.
- Prefer vertical slices over DB-only, API-only, or UI-only issues unless an enabling task is explicitly justified.
- For CLI or no-browser work, a vertical slice may be local fixture/input -> command/API boundary -> report artifact/stdout/log -> non-browser runtime evidence; do not require screenshots or browser QA without a browser surface.
...
- For substantial UI, create or refresh baseline `design.md` early enough to shape PRD/issues, then run the mockup selection pass after implementation planning and before TDD.
- When implementation skills such as `brainstorming`, `writing-plans`, `tdd`, or `verification-before-completion` are missing, mark the dependency as fallback and run a compact Local Implementation Fallback Lane: issue-level implementation design, exact TDD plan, RED/GREEN/REFACTOR, and verification evidence.
