# Saved Workflow Output: New Project Routing

Skill: workflow
Mode: Discovery
Current authority: AGENTS.md present; CONTEXT.md missing; ADRs missing
Runtime adapter: Codex AGENTS.md
Dependencies: inventory required before specialist calls
Next step: Discovery

Artifacts
- Create/update: CONTEXT.md or .scratch/<feature-slug>/WORKFLOW_LOG.md
- Preserve: no generated project tree yet

Validation
- Done when: user/product/domain language is clear enough to ask architecture questions

Open
- Do not call project-structure yet; structure choices are not constrained.
