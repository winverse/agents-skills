...
- When `workflow` lacks implementation helper skills, mark fallback and create issue-level implementation design, exact TDD plan, RED/GREEN/REFACTOR evidence, and non-browser runtime evidence when no browser surface exists.
- When `workflow` touches tools, MCP, external APIs, write-capable automation, network access, or untrusted content, record the Agent Tool And Security Risk Gate before implementation.
- In this repo, `/goal` references mean Claude Code's `/goal` feature. When `workflow` runs long tasks on Claude Code, propose a goal condition with measurable end state, check evidence, constraints, and turn/time bound; on other runtimes, write the same content as a completion checklist.
- When `workflow` needs repeatable behavior checks, call `agent-eval-harness` as a separate validation layer and seed cases for workflow routing, dependency inventory, `project-structure` timing, PRD settings, UI mockup selection, CLI/no-browser evidence, MCP/API gate decisions, fallback lane, project setup verification, completion/ship mapping, document sync, artifact hygiene, and goal condition quality.
...
- When using `transcript-polisher`, treat scripts as discovery or validation helpers only, never as the prose editing engine; split text by coherent sections, delegate one section per subagent when runtime permits, preserve lecture examples and generated awkward-output examples, and report Claude `/goal`-style completion evidence for long reviews.
