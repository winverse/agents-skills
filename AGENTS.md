...
- Use $skill-to-html at `skills/skill-to-html/SKILL.md` whenever a skill is created, installed, forked, or updated, so the skill folder gets a diagram-rich `skill.html` beside `SKILL.md`; optimize only for PC desktop, ignore mobile/tablet layout, and check SVG arrow endpoints, wide table placement, overflow, and text overlap when the visual structure changes.
...
- Use `skills/skill-to-html/SKILL.md` when creating or revising a skill's `skill.html`; optimize only for PC desktop, ignore mobile/tablet layout, keep wide scope tables out of narrow 2-column layouts, and make SVG arrows terminate at visible nodes.
