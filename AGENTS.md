...
- Use $course-evaluator at `skills/course-evaluator/SKILL.md` when asked to grade student answer CSVs, spreadsheets, assignment responses, course quizzes, rubric-based submissions, or educational feedback files into one Markdown file per student.
...
- When using `course-evaluator`, parse CSV or spreadsheet inputs structurally, default to `평가/학생명.md` per student, keep score leniency separate from comment quality, and verify student count, file count, score totals, missing answers, and repeated stock phrases before completion.
