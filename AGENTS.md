...
- Use $project-workflow at `skills/project-workflow/SKILL.md` when asked to start a new project or major initiative; shape a raw idea into domain docs, product challenge notes, ADR, `design.md`, PRD, initial issue backlog, project setup verification, parallel work claims, or a ready handoff to spec implementation; or prevent agents from coding before product and architecture boundaries are clear.
- Use $spec-workflow at `skills/spec-workflow/SKILL.md` when asked to implement an existing PRD, issue, spec, bug report, acceptance criteria, ADR, or `design.md` through planning, work claim preflight, TDD or characterization checks, review, QA/runtime evidence, document sync, and completion reporting.
...
- Use $warp-automation at `skills/warp-automation/SKILL.md` when Warp should use the assistant response as the visible title; format the reply so the first line is a short summary of the user's current prompt.
...
- Treat `project-workflow` plus `spec-workflow` as the Workflow suite: setup writes durable authority, a lightweight `.scratch/<slug>/workflow-state.md` cache, and when parallel sessions are possible a `.scratch/<slug>/work-claims.md` coordination artifact, while implementation reads and updates those artifacts with evidence and open questions.
...
- When `spec-workflow` sees `work-claims.md`, it must confirm the current lane owns the intended write set before production edits. If another active lane owns the same file or module, stop with an overlap block; shared/hotspot files are edited only by the named integration owner.
...
- When using `warp-automation`, do not run terminal or config workflows unless the user separately asks for that work. Begin the reply with a plain first-line title that summarizes the current user prompt in 32 characters or fewer; do not use `Q:`, `Prompt:`, markdown headings, secrets, private URLs, or full local paths in that title, then leave the second line blank and continue the answer on the third line.
